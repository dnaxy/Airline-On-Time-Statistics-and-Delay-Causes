# the code is forked from Matt Leonawicz,
# on https://gist.github.com/leonawicz/0fab3796b02a62b7f3bd0c02a171f0b7
# and then make adjustment for this project
...
valueBox <-
  function (value,
    ...
    if (!is.null(icon)) {
      if (!icon$name %in% c("i", "img"))
        stop("'icon$name' must be 'i' or 'img'.")
      iconClass <- if (icon$name == "i")
        "icon-large"
      else
        "img"
    }
...
icon <- function (name, class = NULL, lib = "font-awesome") {
  if (lib == "local") {
    if (is.null(name$src))
      stop(
        "If lib='local', 'name' must be a named list with a 'src' element
        and optionally 'width' (defaults to 100%)."
      )
    if (is.null(name$width))
      name$width <- "100%"
    return(tags$img(
      class = "img img-local",
      src = name$src,
